...
void LidarFeatureDetector::estimateCornersFromBoardParams(Eigen::Affine3f& board_pose, Eigen::Vector3f& board_size, vector<PointXYZ>& corners)
{
    corners.clear();
...
void LidarFeatureDetector::refineCorners(vector<Vector3d>& corners_3d, vector<PointCloud<PointXYZ>>& line_points_3d)
{
...
bool LidarFeatureDetector::isValidCorners(vector<PointXYZ>& corners)
{
...
bool LidarFeatureDetector::extractPlaneCloud(PointCloud<PointXYZ>::Ptr &input_cloud, PointCloud<PointXYZ>::Ptr &plane_pcd)
{
    m_chessboard_extractor.pass_filter(input_cloud); // 带通滤波
    vector<PointIndices> indices_clusters;
    m_chessboard_extractor.pcd_clustering(input_cloud, indices_clusters); // 聚类
...
bool LidarFeatureDetector::extractPlaneCloud1(PointCloud<PointXYZ>::Ptr &input_cloud,
